# maxtellar — Consolidated Specification v2.0

**App name:** maxtellar (a time-management app)
**Date:** 2026-07-08 · **Status:** Design complete (3 grilling sessions consolidated)
**Supersedes:** v1, `SPEC.md`, `features/*.feature` (early generic drafts, deleted 2026-07-09)

> This is the single source of truth. Rule tags (G#, E#, R#) trace to the design-session log
> in the plan file. No code exists yet; this spec is the contract the build will follow.

---

## Parts

- [PART I — PHILOSOPHY & SOUL](01-philosophy.md)
- [PART II — DATA MODEL](02-data-model.md)
- [PART III — SCHEDULER](03-scheduler.md)
- [PART IV — DAY, WEEK & CEREMONIES](04-day-week-ceremonies.md)
- [PART V — QUOTAS, POMODORO, ALARMS](05-quotas-pomodoro-alarms.md)
- [PART VI — VIEWS](06-views.md)
- [PART VII — ENGINEERING](07-engineering.md)
- [PART VIII — MVP BOUNDARY](08-mvp-boundary.md)
- [PART IX — LEGACY SYSTEM FINDINGS (the 8-yr Sheet + AppScript, studied 2026-07-08)](09-legacy-findings.md)
- [PART X — RESIDUAL OPEN ITEMS (safe to settle during build)](10-open-items.md)
