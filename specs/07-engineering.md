...
| 2026-07-16 | **react-big-calendar** 1.20.0 (MIT, 851k dl/wk) | **Adopt** — Hybrid | Powers the **Week Plan / Calendar** 7-column grid (the Day timeline stays hand-rolled — its now-seam duality, edge-anchoring and seam physics are locked §6 laws). Placement authority stays with core `settle`/`weekPreview`; RBC only renders. Cost accepted knowingly (measured on the R7 spike): **+170kB JS / +55kB gzip, +14kB CSS**, and a direct dependency on a date library (`dayjs`) for RBC's mandatory localizer. **Cross-midnight blocks render as a true span across columns** (`showMultiDayTimes` + `allDayAccessor={() => false}`) — NOT the all-day banner row, which would strip the day's head sleep off the time axis entirely. Conflicts use RBC's side-by-side layout; the §4.6 preview **notice** carries the signal (it always did — the geometry was never the law). |
| 2026-07-16 | **@tanstack/react-virtual** 3.14.6 (MIT, 14.3M dl/wk) | **Adopt** — Hybrid (headless) | Windows the **History** list, which is unbounded by design (it grows for the life of the app). Headless: it computes offsets, we keep our own markup/tokens. The day-grouped render is FLATTENED to one item list (headings + rows) so headings virtualize too — a nested group render can't be windowed without measuring every group. Rows vary in height, so each is measured (`measureElement`); `estimateSize` is only the pre-measure guess. Verified at 2,100 items: **29 DOM nodes** instead of 2,100, with a full-height spacer so the scrollbar still reflects all of history. |
...
| 2026-07-16 | Radix / vaul dialogs, tooltips | **Keep bespoke** (dialogs); **superseded 2026-07-20** (tooltips, below) | §6 locks the custom drawer chrome. Dialogs: healthy packages, but no behavior gap to close; revisit under the re-check rule when one of those files is next edited. Tooltips: the buy-first re-check rule fired for real 2026-07-20 (see next row) — a left-edge clipping bug exposed exactly the gap this original "no behavior gap" verdict missed. |
| 2026-07-20 | **@floating-ui/react** 0.27.20 (MIT, 18.7M dl/wk) | **Adopt** — Hybrid (headless) | Re-check triggered by a reported tooltip clipping bug (§6's `data-tip` mechanism was pure CSS, `right: 0`/`bottom: 100%` fixed-anchor with two hardcoded per-selector overrides for known-clip cases — any NEW trigger near a container edge had no protection and would silently clip, which is exactly what happened). `computePosition` + `flip()` + `shift()` middleware measures the real viewport and picks a side that fits, live — no more manual overrides to remember. Headless only: positioning math is floating-ui's, the visual card (`paper-raised`, hairline, `ink-soft` 11px, `--shadow-2`, 0.5s dwell) stays ours. Migrated via ONE new file (`TooltipHost.tsx`, mounted once at the app root) rather than touching the 125+ existing `data-tip="…"` call sites — the attribute API is unchanged everywhere, only the rendering mechanism moved. See `docs/design-tokens.md`'s Tooltips section. |
